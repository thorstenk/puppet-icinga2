  ...
  on_supported_os.each do |os, facts|
    ...
    context "#{os} with import => [bar, baz], apply => desc in host.vars.bar, assign => [desc.bar]" do
      let(:params) do
        {
          :import       => ['bar', 'baz'],
          :apply        => 'desc in host.vars.bar',
          :apply_target => 'Host',
          :assign       => ['desc.bar'],
          :attrs        => {'vars' => 'vars + desc'},
          :object_type  => 'foo',
          :target       => '/bar/baz',
          :order        => '10',
          :attrs_list   => ['vars']
    ...
    context "#{os} with apply => desc => config in host.vars.bar" do
      let(:params) do
        {
          :apply        => 'desc => config in host.vars.bar',
          :apply_target => 'Host',
          :attrs        => {'vars' => 'vars + desc + !config.bar'},
          :object_type  => 'foo',
          :target       => '/bar/baz',
          :order        => '10',
          :attrs_list   => ['vars']
    ...
    context "#{os} with assign => [ host.vars.os == bar && host.address, generic-host in host.templates]" do
      let(:params) do
        {
          :assign      => ['host.vars.os == bar && host.address', 'generic-host in host.templates'],
          :object_type => 'foo',
          :target      => '/bar/baz',
          :order       => '10'
        }
    ...
    context "#{os} with ignore => [ NodeName != baz || !host.display_name]" do
      let(:params) do
        {
          :ignore      => ['NodeName != baz || !host.display_name'],
          :object_type => 'foo',
          :target => '/bar/baz',
          :order => '10'
        }
    ...
    context "#{os} with attrs => { vars => { bar => unparsed string } }" do
      let(:params) do
        {
          :attrs       => { 'vars' => { 'bar' => '-:"unparsed string"' } },
          :object_type => 'foo',
          :target      => '/bar/baz', :order => '10',
          :attrs_list  => ['vars']
        }
      end
    ...
    context "#{os} with attrs => { vars => {key1 => 4247, key2 => value2} }" do
      let(:params) do
        {
          :attrs       => { 'vars' => {'key1' => '4247', 'key2' => 'value2'} },
          :object_type => 'foo',
          :target      => '/bar/baz',
          :order       => '10',
          :attrs_list  => ['vars']
    ...
    context "#{os} with attrs => { vars => {foo => {key1 => 4247, key2 => value2}} }" do
      let(:params) do
        {
          :attrs       => { 'vars' => {'foo' => {'key1' => '4247', 'key2' => 'value2'}} },
          :object_type => 'foo',
          :target      => '/bar/baz',
          :order       => '10',
          :attrs_list  => ['vars']
    ...
    context "#{os} with attrs => { vars => {foo => {bar => {key => 4247, key2 => value2}}} }" do
      let(:params) do
        {
          :attrs       => { 'vars' => {'foo' => { 'bar' => {'key1' => '4247', 'key2' => 'value2'}}} },
          :object_type => 'foo',
          :target      => '/bar/baz',
          :order       => '10',
          :attrs_list  => ['vars']
    ...
    context "#{os} with attrs => { vars => {foo => {bar => [4247, value2]}} }" do
      let(:params) do
        {
          :attrs       => { 'vars' => {'foo' => { 'bar' => ['4247', 'value2']}} },
          :object_type => 'foo',
          :target      => '/bar/baz',
          :order       => '10',
          :attrs_list  => ['vars']
    ...
    let(:params) do
      {
        :import       => ['bar', 'baz'],
        :apply        => 'desc in host.vars.bar',
        :apply_target => 'Host', :assign => ['desc.bar'],
        :attrs        => {'vars' => 'vars + desc'},
        :object_type  => 'foo',
        :target       => 'C:/bar/baz',
        :order        => '10',
